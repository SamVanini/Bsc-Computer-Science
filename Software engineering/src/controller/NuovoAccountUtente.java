...
public class NuovoAccountUtente implements Initializable {

    @FXML
    private Label errPassword;
    @FXML
    public Label errNomeG;
    @FXML
    public Label errTelefonoG;
    @FXML
    public Label errCognomeG;
    @FXML
    public Label errMailG;
    @FXML
    public Label errHobby;
    @FXML
    ...
    private boolean controllaNumeroCivico(String string) {
        ...
        return (ch != '0') && (ch <= 'a' || ch >= 'z') && (ch <= 'A' || ch >= 'Z');
    }
    ...
    public void creazioneAccount(ActionEvent actionEvent) throws IOException {
        Main nuovaScena = new Main();
        ...
        if(Nome.getText().isBlank() || Cognome.getText().isBlank()|| Eta.getText().isBlank() ||
            ...
            if(Allergia1.getSelectionModel().getSelectedItem() != null && Allergia2.getSelectionModel().getSelectedItem() != null)
                ...
                GenitoriDAO.insertGenitori(NomeG.getText(), CognomeG.getText(), TelefonoG.getText(), MailG.getText());
                int idGenitore = GenitoriDAO.selectGenitori(NomeG.getText(), CognomeG.getText(), TelefonoG.getText(), MailG.getText());
                LockDAO.unlock("genitori");
                while(LockDAO.isLocked("ragazzi") == 1);
                LockDAO.lock("ragazzi");
                RagazziDAO.insertRAGAZZI(Nome.getText(), Cognome.getText(), parseInt(Eta.getText()), Via.getText(), NumeroCivico.getText(), Citta.getText(), Telefono.getText(),
                    Mail.getText(), Hobby.getText(), Allergia1.getSelectionModel().getSelectedItem().toString(), Allergia2.getSelectionModel().getSelectedItem().toString(), idGenitore);
                int idRagazzo = RagazziDAO.selectRagazzi(Nome.getText(), Cognome.getText(), parseInt(Eta.getText()), Via.getText(), NumeroCivico.getText(), Citta.getText(), Telefono.getText(),
                    Mail.getText(), Hobby.getText(), Allergia1.getSelectionModel().getSelectedItem().toString(), Allergia2.getSelectionModel().getSelectedItem().toString(), idGenitore);
                LockDAO.unlock("ragazzi");
                while(LockDAO.isLocked("credenziali") == 1);
                LockDAO.lock("credenziali");
                CredenzialiDAO.insertCredenziali(Mail.getText(), Password.getText());
                LockDAO.unlock("credenziali");
                utente = Utente.getInstance();
                utente.setId(idRagazzo);
                ...
                utente.setGenitore(idGenitore);
                genitore = Genitori.getInstance();
                genitore.setId(idGenitore);
