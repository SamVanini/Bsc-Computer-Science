...
static bool_t state_safe(size_t n, size_t m)
{
    ...
    while (arr_ne(finish, all_true, n))
    {
        ...
        for (i = 0; i < n && (finish[i] == 1UL || arr_g_any(need[i], work, m)); i++);
        if (i == n)
        {
            // Free memory.
            kfree(work);
...
deadlock_status_t request(uint32_t *req_vec, size_t task_i, size_t n, size_t m)
{
    ...
    // Try to allocate resources.
   	available = arr_sub(available, req_vec, m);
	alloc[task_i] = arr_add(alloc[task_i], req_vec, m);
	need[task_i] = arr_sub(need[task_i], req_vec, m);

    // Check safe state
    if (!state_safe(n, m))
    {
        // Restore previous allocation.
        available = arr_add(available, req_vec, m);
		alloc[task_i] = arr_sub(alloc[task_i], req_vec, m);
		need[task_i] = arr_add(need[task_i], req_vec, m);
        return WAIT_UNSAFE;
    }
    return SAFE;
}
